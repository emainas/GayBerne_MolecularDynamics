...
void GB::_savedata(const Vec2d* var, const string& fn) const{
  ofstream out("./N128Rho028Temp1Time10e5/GB_"+fn+".txt");
  for (int i=0; i<N; i++) {
    out << var[i];
  }
  out.close();
...
void GB::_loaddata(Vec2d* var, const string& fn) {
  ifstream in("./N128Rho028Temp1Time10e5/GB_"+fn+".txt");
  for (int i=0; i<N; i++) {
    in >> var[i];
  }
  in.close();
...
void GB::susceptibility1(){
  ...
  for (int i=0; i<N; i++) {
    for (int j=0; j<N; j++) {
        susc1 += (2*phi[i]*phi[i] - 1)*(2*phi[j]*phi[j] - 1);
    }
  }
  chi1 = susc1/N;
}
...
void GB::g_corr() {
  ...
  ofstream g("./N128Rho028Temp1Time10e5/g.txt");
...
void GB::g_orient() {
  ...
  ofstream gor("./N128Rho028Temp1Time10e5/gor.txt");
  for (int bin = 0; bin < maxbin; ++bin) {
    double rlow = bin * dr;
    double rup = (bin + 1) * dr;
    double nideal =
...
double GB::run(int is_init_file){
  ofstream energy1("./N128Rho028Temp1Time10e5/energy1.txt");
  ...
  if (is_init_file==0) {
    ...
    for (int i=0; i<initial_steps; i++) {
      ...
      if ((step_count+1)%10==0) {
        ...
        calc_order();
        susceptibility1();
        energy1<<index<<" "<< potentialEnergy()<<" "<<kineticEnergy()<<" "<<rotKE()<<" "<<transKE()<<" "<<order<<" "<<chi1<<endl;
      }
    }
...
//    savedata();                       //cnm
  ...
  ofstream energy2("./N128Rho028Temp1Time10e5/energy2.txt");
  
  int index2;
  ofstream forder("./N128Rho028Temp1Time10e5/order.txt");
  ofstream fsusc1("./N128Rho028Temp1Time10e5/susc1.txt");
  ...
  for (int i=0; i<data_steps; i++) {
    ...
    if ((step_count+1)%10==0) {
      ...
      Total[index2] = potentialEnergy() + kineticEnergy();
      calc_order();
      S[index2] = order;
      ...
      energy2<<index<<" "<< potentialEnergy()<<" "<<kineticEnergy()<<" "<< potentialEnergy() + kineticEnergy()<<endl;
      forder<<index2<<" "<< order<<endl;
      fsusc1<<index2<<" "<< chi1<<endl;
      savedata();      
