...
void GB::_savedata(const Vec2d* var, const string& fn) const{
  ofstream out("./N128Rho023Temp1-eos/GB_"+fn+".txt");
  for (int i=0; i<N; i++) {
    out << var[i];
  }
  out.close();
...
void GB::_loaddata(Vec2d* var, const string& fn) {
  ifstream in("./N128Rho023Temp1-eos/GB_"+fn+".txt");
  for (int i=0; i<N; i++) {
    in >> var[i];
  }
  in.close();
...
void GB::calc_order(){
 ...
 for (int i=0; i<N; i++) {
   for (int j=0; j<N; j++) {
     tempvar += 2*pow(ort[i].y*ort[j].y + ort[i].x*ort[j].x, 2) - 1; 
   }  
 }
 order = (pow(tempvar/(N*N) , 0.5));
}
...
void GB::g_corr() {
  ...
  ofstream g("./N128Rho023Temp1-eos/g.txt");
...
void GB::g_orient() {
  ...
  ofstream gor("./N128Rho023Temp1-eos/gor.txt");
  for (int bin = 0; bin < maxbin; ++bin) {
    double rlow = bin * dr;
    double rup = (bin + 1) * dr;
    double nideal =
...
void GB::eos(){
  ...
  ofstream eosfile("./N128Rho023Temp1-eos/eosfile.txt");
  //ofstream potfile("./N128Rho0285Temp1Time10e4-EOSpos/pot.txt");
...
double GB::run(int is_init_file){
  ofstream energy1("./N128Rho023Temp1-eos/energy1.txt");
...
//    savedata();                       //cnm
  ...
  int index2;
  ofstream forder("./N128Rho023Temp1-eos/order.txt");
