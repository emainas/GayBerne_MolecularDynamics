...
void GB::integration() {
  ...
  double n = 200;
  double h = PI/n; // Same step for all algorithms
  ...
  for (int a = 1; a < 5001; ++a) { 
    ...
    for (int k = 1; k <= n/4; k++) {
      for (int j = 1; j <= n/4; j++) {          
        boole += 7*( 7*f(r, (4*k-4)*h, (4*j-4)*h) + 32*f(r, (4*k-4)*h, (4*j-3)*h) + 12*f(r, (4*k-4)*h, (4*j-2)*h) + 32*f(r, (4*k-4)*h, (4*j-1)*h) + 7*f(r, (4k-4)*h, (4j)*h)) 
        + 32*( 7*f(r, (4*k-3)*h, (4*j-4)*h) + 32*f(r, (4*k-3)*h, (4*j-3)*h) + 12*f(r, (4*k-3)*h, (4*j-2)*h) + 32*f(r, (4*k-3)*h, (4*j-1)*h) + 7*f(r, (4k-3)*h, (4j)*h))
        + 12*( 7*f(r, (4*k-2)*h, (4*j-4)*h) + 32*f(r, (4*k-2)*h, (4*j-3)*h) + 12*f(r, (4*k-2)*h, (4*j-2)*h) + 32*f(r, (4*k-2)*h, (4*j-1)*h) + 7*f(r, (4k-2)*h, (4j)*h))
        + 32*( 7*f(r, (4*k-1)*h, (4*j-4)*h) + 32*f(r, (4*k-1)*h, (4*j-3)*h) + 12*f(r, (4*k-1)*h, (4*j-2)*h) + 32*f(r, (4*k-1)*h, (4*j-1)*h) + 7*f(r, (4k-1)*h, (4j)*h))
        + 7*( 7*f(r, (4*k)*h, (4*j-4)*h) + 32*f(r, (4*k)*h, (4*j-3)*h) + 12*f(r, (4*k)*h, (4*j-2)*h) + 32*f(r, (4*k)*h, (4*j-1)*h) + 7*f(r, (4k)*h, (4j)*h));
      } 
    } 
  ...
  if (is_init_file==0) {
    ...
    for (int i=0; i<initial_steps; i++) {
      ...
      if ((step_count+1)%10==0) {
        ...
        calc_order();
        energy1<<index<<" "<< potentialEnergy()<<" "<<kineticEnergy()<<" "<<rotKE()<<" "<<transKE()<<" "<<order<<endl;
      }
    }
...
//    savedata();                       //cnm
  ...
  int index2;
  ofstream forder("./N128Rho0285Temp1-eps/order.txt");
