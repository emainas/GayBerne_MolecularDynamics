  ...
  for (int a = 1; a < 4001; ++a) { 
    ...
    for (int k = 0; k < n; k++) {
      for (int j = 0; j < n; j++) {          
        ...
        c88 += H(8, r, j*h, k*h) + H(8, r, (j+1)*h, k*h) + H(8, r, j*h, (k+1)*h) + H(8, r, (j+1)*h, (k+1)*h);
        c11 += H(10,r, j*h, k*h) + H(10,r, (j+1)*h, k*h) + H(10,r, j*h, (k+1)*h) + H(10,r, (j+1)*h, (k+1)*h);
      } 
    } 
    fcnn<<r<<"  "<<C2*trap<<" "<<C2*c22<<" "<<C2*c44<<" "<<C2*c66 <<" "<<C2*c88<<" "<<C2*c11<<" "<<endl;
    //trap = 0.0;
    c22 = 0.0;
    c44 = 0.0;
    c66 = 0.0;
